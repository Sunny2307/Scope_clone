...
export default function App() {
    ...
    const validateStep = () => {
        ...
        if (currentStep === 1) {
            ...
            if (!/\S+@\S+\.\S+/.test(formData.personalEmail)) newErrors.personalEmail = 'Email address is invalid';
        }
        setErrors(newErrors);
    ...
    // If form is submitted successfully, show a success message
    if (isSubmitted) {
        return (
        ...
        <div className="w-screen bg-gray-50 font-sans flex items-center justify-center min-h-screen p-4">
            <div className="w-full max-w-4xl bg-white rounded-2xl shadow-lg p-8">
                ...
                <div className="flex justify-between mt-8">
                    {currentStep > 1 && (
                        <button onClick={prevStep} className="flex items-center px-6 py-2 bg-gray-200 text-white font-semibold rounded-lg hover:bg-gray-300 transition-colors">
    ...
    return (
        <div className="space-y-6 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <InputField label="Registration Date" name="registrationDate" type="date" value={formData.registrationDate} onChange={handleChange} />
                <SelectField label="Admission Year" name="admissionYear" value={formData.admissionYear} onChange={handleChange}>
                    <option value="">Select Year</option>
                    {Array.from({ length: 10 }, (_, i) => new Date().getFullYear() - i).map(year => (
                        <option key={year} value={year}>{year}</option>
                    ))}
                </SelectField>
                <SelectField label="Current Semester" name="currentSemester" value={formData.currentSemester} onChange={handleChange}>
...
// Component for Contact Info step
const ContactInfo = ({ formData, handleChange, errors }) => (
    <div className="space-y-6 animate-fadeIn">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <InputField label="Institutional Email" name="institutionalEmail" type="email" value={formData.institutionalEmail} onChange={handleChange} />
        </div>
    </div>
);

// Component for Academic Info step
const AcademicInfo = ({ formData, handleChange, errors }) => (
    <div className="space-y-6 animate-fadeIn">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <InputField label="Name of Guide" name="nameOfGuide" value={formData.nameOfGuide} onChange={handleChange} />
            <InputField label="UGC ID" name="ugcId" value={formData.ugcId} onChange={handleChange} />
...
// Component to review the form data
...
const SelectField = ({ label, name, value, onChange, error, required, children }) => (
    <div>
        <label htmlFor={name} className="block text-sm font-medium text-gray-700 mb-1">
            {label} {required && <span className="text-red-500">*</span>}
        </label>
